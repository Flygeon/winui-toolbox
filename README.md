...
- **全局搜索**：任意界面按 **Ctrl+K** 唤起，键盘上下键 + 回车跳转。
- **工具输入记忆**：切换工具/页面输入不丢失，重启应用后仍保留。
- **强调色跟随系统**：自动读取 Windows 系统强调色应用到界面。
- **系统托盘**：常驻托盘图标（左键显示、右键菜单），支持「最小化到托盘」与「开机自启」。
- **文件保存目录**：可指定默认保存目录，转换/下载结果直接存入并可一键「打开目录」。
- **主题切换**：浅色 / 深色 / 跟随系统，设置持久化。
- **可折叠侧边栏**：按「开发者工具 / 办公与文档 / 多媒体 / 系统与效率」四模块分组导航，可「始终展开」。
- **单元测试**：核心纯逻辑（diff / CSV / 子网计算）由 Vitest 覆盖。
...
| 多媒体 | 图片格式转换（8 种格式 / Rust 后端）、图片压缩（含 PNG 无损优化）、图像变换（裁剪/旋转/翻转/缩放）、图像调整（亮度/对比度/饱和度/色相/伽马/色温）、EXIF 工具（查看/剥离）、音视频转换（FFmpeg）、图标生成、GIF 动图（FFmpeg）、音频格式转换（FFmpeg） | ✅ 已完成 |
| 多媒体 | OCR 文字识别、截图与录屏 | 🚧 规划中（第三阶段） |
| 系统与效率 | 番茄钟 / 白噪音、剪贴板历史、端口占用查询、环境变量管理、硬件监控、系统托盘 + 开机自启 + 最小化到托盘 | ✅ 已完成 |
| 系统与效率 | 系统清理、快捷键启动、局域网快传 | 🚧 规划中（第四阶段） |

**FFmpeg（不内置）**：音视频工具需要本地 FFmpeg。可在「设置 → FFmpeg」自动扫描 PATH、手动选择本地 ffmpeg.exe；未检测到时提示下载地址。

**已实现的增强能力**：主页（搜索/最近/收藏/模块卡片）、全局 Ctrl+K 搜索、工具收藏 ⭐、工具输入记忆、窗口尺寸与位置记忆、强调色跟随系统、侧边栏「始终展开」开关。
