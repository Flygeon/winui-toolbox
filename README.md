...
## 🗂️ 当前进度（第一阶段 + 第二阶段核心子集）

| 模块 | 工具 | 状态 |
| --- | --- | --- |
| 开发者工具 | 时间戳转换、编码/解码、JSON 格式化、哈希/加解密（含文件哈希）、二维码（生成+解析）、UUID/随机密码、JWT 调试、正则测试（含替换）、Cron 表达式、IP/网络工具（本机 IP+子网计算） | ✅ 已完成 |
| 办公与文档 | 文本处理、敏感信息脱敏、文本比对 Diff | ✅ 已完成 |
| 办公与文档 | Markdown 编辑器、Markdown 转 PDF/Word、表格数据转换 | 🚧 规划中（第二阶段） |
| 办公与文档 | PDF 工具集 | 🚧 规划中（第三阶段） |
| 多媒体 | 图片/音视频转换、图标生成、GIF、OCR、截图录屏等 | 🚧 规划中（第三阶段） |
| 系统与效率 | 硬件监控、系统清理、剪贴板历史、端口查询等 | 🚧 规划中（第四阶段） |

**已实现的增强能力**：工具收藏（⭐）/最近使用/记住最后使用的工具、窗口尺寸与位置记忆、侧边栏「始终展开」开关、工具搜索。
...
| 持久化 | `tauri-plugin-store`（主题、侧边栏状态） |
| 工具库 | `crypto-js`（哈希/加解密）、`qrcode`（二维码） |
