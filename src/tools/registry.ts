...
export const toolModules: ToolModuleDef[] = [
  ...
  },
  {
    ...
    icon: "",
    tools: [
      ready("monitor", "硬件监控", "CPU、内存、磁盘、运行时长实时监控", "", () => import("@/tools/monitor/index.vue")),
      planned("cleanup", "系统清理", "清理临时文件、浏览器缓存", "", 4, ["扫描临时文件", "浏览器缓存清理", "回收站 / 大文件分析"]),
      planned("hotkey", "快捷键启动", "全局热键快速唤出工具", "", 4, ["注册全局热键", "热键呼出指定工具", "开机自启"]),
      ready("clipboard", "剪贴板历史", "记录、固定和管理剪贴板内容", "", () => import("@/tools/clipboard/index.vue")),
      planned("lan", "局域网快传", "局域网点对点文件传输", "", 4, ["局域网设备发现", "P2P 文件传输", "传输进度与断点续传"]),
      ready("env", "环境变量管理", "可视化查看、编辑、备份用户环境变量", "", () => import("@/tools/env/index.vue")),
      ready("port", "端口占用查询", "查看端口占用并一键结束进程", "", () => import("@/tools/port/index.vue")),
